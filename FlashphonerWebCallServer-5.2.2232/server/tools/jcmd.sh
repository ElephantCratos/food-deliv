#!/bin/bash
#
# Script to execute some Java commands in Flashphoner WebCallServer environment
#
...
function declareGlobals() {
  ...
  JCMD_LOG_FILE="$WCS_HOME/logs/jcmd.log"
  REPORTS_HOME="$WCS_HOME/report"
  WCS_PID=""
}

# Get log file name
function getJcmdLogFile() {
  echo "$JCMD_LOG_FILE"
}
...
# Launch GC for the pid
function do_gc() {
  local pid=$1
  local result=0

  displayInfo "Launch garbage collection for the process $pid"
  runGc $pid
  result=$?
  if [ $result -eq 0 ]; then
    displayInfo "Garbage collection command executed successfully"
  else
    displayError "Garbage collection failed: error $result ($(strError $result))"
  fi
  return $result  
}
...
function do_jstack() {
  ...
  displayInfo "Collect jstack dump $jstackFile for the process $pid"
  getJstack $pid $jstackFile
  result=$?
  if [ $result -eq 0 ]; then
    displayInfo "Jstack dump $jstackFile collected successfully"
  else
    displayError "Jstack dump collection failed: error $result ($(strError $result))"
  fi
  return $result  
}
...
function do_dump() {
  ...
  displayInfo "Collect heap dump $dumpFile for the process $pid"
  runGc $pid && getHeapDump $pid $dumpFile
  result=$?
  if [ $result -eq 0 ]; then
    displayInfo "Heap dump $dumpFile collected successfully"
  else
    displayError "Heap dump collection failed: error $result ($(strError $result))"
  fi
  return $result
}
...
# Java flight recording collection functions
...
function do_jfr() {
  ...
  displayInfo "Start flight recording to $jfrFile for the process $pid"
  getJfr $pid $duration $jfrFile
  result=$?
  if [ $result -eq 0 ]; then
    displayInfo "Flight recording started successfully. File $jfrFile should be ready in $duration"
  else
    displayError "Flight recording startup failed: error $result ($(strError $result))"
  fi
  return $result
}
...
function usage() {
  ...
  echo "Execute a Java command applying to the WebCallServer instance"
  echo -e "  gc\t\t launch garbage collection"
  echo -e "  jstack\t collect jstack dump"
  echo -e "  --output\t output file name ($(getReportsHome)/DATE/PID.jstack by default)"
  echo -e "  dump\t\t collect heap dump"
  echo -e "  --output\t output file name ($(getReportsHome)/DATE/PID.dump by default)"
  echo -e "  jfr\t\t launch Java flight recording"
  echo -e "  --output\t output file name ($(getReportsHome)/DATE/PID.jfr by default)"
  echo -e "  --duration\t flight recording duration (60s by default)"
  echo -e "  \t\t use s for seconds, m for minutes"
...
function main() {
  ...
  local defaultFile=""
  local defaultDir=""
  local defaultDuration="60s"
  local msgMustBeRoot="The script must be run as root"

  declareGlobals
  declareLog $(getJcmdLogFile)

  # Script sholud run from root
  if [[ "$(getCurrentUser)" != "root" ]]; then
  ...
  if isUser $(getUser) && isScriptRunningFromRoot; then
    touch $(getJcmdLogFile)
    setOwner $(getUser) $(getJcmdLogFile)
  fi
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      jstack)
        command=$1
        defaultFile="$pid.jstack"
        ;;
      dump)
        command=$1
        defaultFile="$pid.hprof"
        ;;
      jfr)
        command=$1
        defaultFile="$pid.jfr"
      ...
      --debug)
        enableDebug;;
      *)
        displayError "Wrong parameter $1"
        usage
        return $EINVAL
        ;;
    esac
  ...
  if [ -z $command ]; then
    displayError "No command to execute"
    usage
    return $EINVAL
  else
    if [[ $command != "gc" ]] && [[ -z $output ]]; then
       defaultDir=$(getReportsHome)/report-$(date +%Y-%m-%d-%H-%M-%S)
       [ ! -d $defaultDir ] && mkdir -p $defaultDir
       isUser && setOwner $(getUser) $defaultDir
       output=$defaultDir/$defaultFile
    fi
    if [[ $command == "jfr" ]] && [[ -z $duration ]]; then
       duration=$defaultDuration
    fi
    do_$command $pid $output $duration
    return $?
  fi
}
