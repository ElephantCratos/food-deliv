            ...
            @if ($orders->isEmpty())
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg mb-4">
                    <div class="p-6 bg-white border-b border-gray-200">
                        <p class="text-black">You have no assigned orders.</p>
                    </div>
                </div>
            @else
                <div class="mb-6 p-4 bg-blue-50 rounded-lg">
                    <p class="font-semibold">Courier: {{ $courier->name }}</p>
                    <p>Assigned orders: {{ $orders->count() }}</p>
                </div>
                        ...
                        <div class="mt-4 pt-4 border-t border-gray-200">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div class="text-right">
                                    <p class="text-sm text-gray-500">Total Items: {{ $order->positions->sum('quantity') }}</p>
                                    <p class="text-lg font-bold">Total: ${{ number_format($order->price, 2) }}</p>
                                </div>
                            </div>
                        </div>
